...
// ─── Building dimensions: Sleek launch-pad tower ───────
const BW = 110, BD = 55, BH = 130;   // Base (widest)
const MW = 90,  MD = 50, MH = 120;   // Mid (text)
const TW = 65,  TD = 42, TH = 90;    // Top (narrowest)
...
// ─── Rocket (Arki logo element) ─────────────────────────
function createRocket(accent: string): THREE.Group {
  const g = new THREE.Group();
  const mat = new THREE.MeshStandardMaterial({
    color: accent, emissive: accent, emissiveIntensity: 3, toneMapped: false,
  });

  // Rocket body shape — extruded for 3D pixel feel
  const shape = new THREE.Shape();
  // Rocket pointing up, ~30 units tall
  shape.moveTo(0, 15);       // nose tip
  shape.lineTo(5, 8);        // right nose slope
  shape.lineTo(5, -5);       // right body
  shape.lineTo(9, -12);      // right fin tip
  shape.lineTo(5, -8);       // right fin inner
  shape.lineTo(5, -10);      // right bottom
  shape.lineTo(-5, -10);     // left bottom
  shape.lineTo(-5, -8);      // left fin inner
  shape.lineTo(-9, -12);     // left fin tip
  shape.lineTo(-5, -5);      // left body
  shape.lineTo(-5, 8);       // left nose slope
  shape.closePath();
...
export default function ArkiBuilding({
  themeAccent,
  themeWindowLit,
  themeFace,
  ...
  // Mid textures — "ARKI" text centered
  const mRows = 10;
  const mTxRow = Math.floor((mRows - TEXT_BM.length) / 2);
  const mFront = useMemo(() =>
    createGlassTex(MIN_COLS, mRows, 41, themeWindowLit, windowOff, themeFace, themeAccent, TEXT_BM, txCol, mTxRow),
    [themeWindowLit, windowOff, themeFace, themeAccent, mRows, txCol, mTxRow],
  );
  const mFrontB = useMemo(() =>
    createGlassTex(MIN_COLS, mRows, 97, themeWindowLit, windowOff, themeFace, themeAccent, TEXT_BM, txCol, mTxRow),
    [themeWindowLit, windowOff, themeFace, themeAccent, mRows, txCol, mTxRow],
  );
  const mSide = useMemo(() =>
    createGlassTex(5, mRows, 78, themeWindowLit, windowOff, themeFace),
    [themeWindowLit, windowOff, themeFace, mRows],
  );

  // Base textures
  const bFront = useMemo(() =>
    createGlassTex(MIN_COLS, 9, 72, themeWindowLit, windowOff, themeFace),
    [themeWindowLit, windowOff, themeFace],
  );
  const bSide = useMemo(() =>
    createGlassTex(5, 9, 83, themeWindowLit, windowOff, themeFace),
    [themeWindowLit, windowOff, themeFace],
  );

  // Top textures
  const tFront = useMemo(() =>
    createGlassTex(MIN_COLS, 6, 55, themeWindowLit, windowOff, themeFace),
    [themeWindowLit, windowOff, themeFace],
  );
  const tSide = useMemo(() =>
    createGlassTex(4, 6, 66, themeWindowLit, windowOff, themeFace),
    [themeWindowLit, windowOff, themeFace],
  );
  ...
  const rocket3D = useMemo(() => createRocket(themeAccent), [themeAccent]);
  ...
  useFrame(({ clock }) => {
    ...
    if (rocketRef.current) rocketRef.current.rotation.y = t * 0.4;
  ...
  return (
    <group>
      ...
      {/* ── Rocket (rotating) ── */}
      <group ref={rocketRef} position={[0, antennaY + 32, 0]}>
        <primitive object={rocket3D} />
        <pointLight color={themeAccent} intensity={40} distance={120} decay={2} />
      </group>

      {/* ── Beacon above rocket ── */}
      <mesh ref={beaconRef} position={[0, antennaY + 52, 0]}>
      ...
      <pointLight position={[0, antennaY + 52, 0]} color={themeAccent} intensity={20} distance={100} decay={2} />
